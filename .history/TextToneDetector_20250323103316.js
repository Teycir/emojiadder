  ...
  async analyze(text) {
    ...
    try {
      ...
      for (const paragraph of paragraphs) {
        ...
        // This regex matches sentence endings: period, question mark, or exclamation mark followed by a space or end of string
        ...
        for (const sentence of sentences) {
          ...
          const emojiString = `${toneEmoji} ${metaphorEmoji}`.trim();
          // This regex captures the end punctuation and any trailing space
          const enhancedSentence = sentence.replace(/([.!?]+)(\s|$)/, `$1 ${emojiString}$2`);
